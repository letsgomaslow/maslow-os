...
grep -Fq 'Maslow OS does not assume sign-in succeeded.' "$ROOT/shell/plugins/maslow-ai-setup/Panel.qml" || fail "AI panel treats process exit as verified sign-in"
...
grep -Fq 'Start with Bitwarden, then choose Codex, Claude Code, or Hermes.' "$ROOT/shell/plugins/maslow-ai-setup/Panel.qml" || fail "AI panel does not explain the tested starter flow"
