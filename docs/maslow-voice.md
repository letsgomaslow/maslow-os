...
Current status: the source tree contains an unpublished staging candidate whose default conversation path is Gemini 3.8 Live. It has automated source coverage but has not passed the Lenovo acceptance matrix, package promotion, stable-channel publication, or verified-ISO rebuild gates. The [September 16 Lenovo handoff](handoffs/2026-09-16-lenovo-voice-development-handoff.md) remains the latest installed-device evidence for the earlier GPT-Live package; it does not establish this candidate's hardware behavior. Use the [development index](maslow-development.md) and [Voice testing guide](../agents/skills/voice-development.md) when preparing acceptance.
