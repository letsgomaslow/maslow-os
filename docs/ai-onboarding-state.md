...
Maslow Connect is a separate first-party application and command boundary. The Setup menu opens it through `omarchy connect open`; enrollment and agent configuration use the explicit `omarchy connect ...` commands. Maslow Connect does not write credentials or connection state into this onboarding file, and the Hermes-specific MCP card continues to open only Hermes' own picker.
