...
Current priority: [LiveKit native audio recovery](handoffs/2026-09-14-livekit-native-audio.md) is unfinished and not ready for Lenovo delivery. Unpublished Voice 0.1.4-16 passed 37 focused installed checks and one complete synthetic spoken-handoff scenario: verified Hermes file, audible interruption, concurrent reply and a 60-second soak with zero drops. A separate first-use run failed because Voice transcribed `result.html` as `result.H` and handed Hermes the wrong filename; independent task/file inspection confirms this accuracy defect. A 30 ms synthetic echo profile passed, while a 90 ms profile interrupted a reply and remains unresolved. These scoped results are not hardware or release acceptance. The next work is critical-detail accuracy and bounded echo-timing diagnosis; preserve earlier failures and avoid broad reruns without a concrete change. No publication, OTA, ISO rebuild or Lenovo change follows from this checkpoint.
