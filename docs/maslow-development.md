...
Current priority: the user reports the Hub update and Voice module are visible on Lenovo. Account-setup review found three real LiveKit startup/audio wiring defects in Voice 0.1.2. The [LiveKit setup correction](handoffs/2026-09-13-livekit-setup-correction.md) records Voice 0.1.3 inside Hub 0.3.1, its Update Voice action, build/test evidence and the new unsigned staging-sequence-5 candidate. Installed correction, credential-storage and visual checks passed; the next step is operator signing of the prepared 0.3.1 update, then publication and a real Lenovo conversation test. Published sequence 4 remains unchanged. Live account/audio and the newly reported flat orb appearance remain open; provide test instructions before investigating the orb, as requested. The separate [support-readiness closeout](handoffs/2026-09-12-support-readiness-closeout.md) remains the next infrastructure workstream.
