...
Updated 2026-09-16. This is the current workstream index, not a stable-release announcement. Update this file at each meaningful checkpoint; keep detailed evidence in dated handoffs rather than expanding AGENTS.md into a second backlog.

Current GPT-Live delivery work: [Hub 0.3.2 / exact Voice 0.1.4-9 Lenovo test update](handoffs/2026-09-14-gpt-live-lenovo-test-release.md), authorized by the user after the documentation closeout. Hub and its installed OTA/Voice-update rehearsal passed, including preserved state and native voice selection. The separate fresh Mac synthetic GPT-Live check passed after a tester-only cache fix. Operator signing, immutable package publication and staging sequence 5 promotion are complete at public distribution `f605c57fa2927a44e218d795845e920e31aca2e9`; anonymous download and metadata verification passed at 21:58:46 UTC. The user now reports Lenovo Voice update and OpenAI/Hermes setup completed, but Voice is not working. Hardware acceptance failed; the failing layer and cause are unknown. Troubleshooting is deferred; see the September 16 handoff. No ISO rebuild is needed.
...
Resume Voice through [the Voice development/testing guide](../agents/skills/voice-development.md) and the checkpoint above. Runtime integration is `codex/livekit-setup`; package integration is `codex/linux-live-packages`. Rediscover their worktrees with Git. The private Mac testers on ports 57517/57518/57519 use independent RAM-only account values; do not restart them casually or request keys in chat. The prior credentialed guest cleanup removed guest credentials. The new OTA rehearsal used a separate overlay with no new provider credentials and ended with its owned VM/build containers stopped; its lifecycle and cleanup are recorded in the test-release handoff. Direct named-agent routing remains a proposed simplification, not the implemented Hermes coordinator path. The [support-readiness closeout](handoffs/2026-09-12-support-readiness-closeout.md) remains the separate infrastructure workstream.

Current priority: [Lenovo failure and development handoff](handoffs/2026-09-16-lenovo-voice-development-handoff.md). Use the [portable repository map and Lenovo workspace instructions](lenovo-development.md) to continue directly on the hardware. Source backup is authorized; product merges and new release publication are outside this closeout. LiveKit remains pending.

## Resume in a new context

1. Read this file and the [latest Lenovo handoff](handoffs/2026-09-16-lenovo-voice-development-handoff.md), then the current checkpoint for the requested workstream. For GPT-Live delivery, start with [the Lenovo test release](handoffs/2026-09-14-gpt-live-lenovo-test-release.md); for overall Voice scope, use [the consolidated checkpoint](handoffs/2026-09-14-voice-checkpoint.md); for infrastructure, use [support readiness](handoffs/2026-09-12-support-readiness-closeout.md) and [recovery release evidence](handoffs/2026-09-11-hub-recovery.md). Respect pending work; use older handoffs for provenance, not instructions to repeat tests, signing or device setup.
...
Current delivery checkpoint: [GPT-Live Lenovo test release](handoffs/2026-09-14-gpt-live-lenovo-test-release.md) is published and anonymously verified as Hub 0.3.2 / staging sequence 5, superseding the frozen 0.3.1 candidate for this test. The exact 0.3.0 package remains available for Hub rollback. The September 16 user reports completing the Voice update and OpenAI/Hermes setup on Lenovo, but Voice is not working; exact installed versions and the failing layer remain uninspected. Hardware acceptance failed, with root cause unknown. LiveKit remains pending. A3 physical Lenovo recovery and F1 Observability backend/trace acceptance remain open; physical Voice audio and complete orb interaction are separate gates. R1/R2 remain recommendations, and security design S1 still blocks any promise of agent-hidden secrets.
