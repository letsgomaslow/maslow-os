...
Current priority: the [GPT-Live native integration checkpoint](handoffs/2026-09-13-gpt-live-native-integration.md) supersedes the earlier verification-only checkpoint. Native GPT-Live, 22 voice choices, combined LiveKit setup, the shaded orb, and bounded per-watcher IPC delivery are implemented in `codex/livekit-setup`; unsigned Voice 0.1.4-9 is the current candidate. Installed Maslow passed native conversation during an independent typed Hermes correction, correct artifact verification, and a further 60 seconds of audio with zero provider errors. The normal launcher recovered on retry after one busy-desktop deferral. Rev8's spoken request produced `resume.html` instead of the requested `result.html`; that original accuracy failure remains open. TCG still recorded capture drops and long scheduling delays, so physical smoothness is unproven. Next address spoken precision; hardware audio, cold coordinator startup during conversation, real Linux LiveKit, native Codex/Claude launches, and local inference remain open. No Lenovo delivery is authorized by these local acceptance results.
