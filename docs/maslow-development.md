...
Current priority: [LiveKit native audio recovery](handoffs/2026-09-14-livekit-native-audio.md) continues the [installed handoff checkpoint](handoffs/2026-09-14-livekit-installed-handoff.md). Real spoken LiveKit requests now reach Hermes and produce the requested file in the installed guest. A direct AgentSession audio experiment passed concurrent spoken conversation and a 60-second soak, but the normal package's follow-up during audible interruption still fails. The native audio adapter, cancellation-safe playback cleanup and truthful microphone startup are implemented; Voice 0.1.4-14 is an unpublished local candidate. Preserve the failed gates: package-14 diagnostics show false speaking events during nominal silence, repeated reply cancellation and premature task dispatch that omits later spoken details. An isolated native device/APM silence probe was exactly zero. Next locate the false speech detection and correct turn handling, then pass task-detail accuracy, spoken concurrency and cold coordinator startup with the desktop active before assessing delivery. This is not Lenovo readiness.

The [GPT-Live native integration checkpoint](handoffs/2026-09-13-gpt-live-native-integration.md) remains the earlier provider baseline. Native GPT-Live, 22 voice choices, combined LiveKit setup, the shaded orb and bounded per-watcher IPC delivery are implemented in `codex/livekit-setup`. Installed Maslow passed GPT-Live audio during an independent typed Hermes correction, correct artifact verification and a further 60 seconds of audio with zero provider errors. Rev8's spoken request produced `resume.html` instead of `result.html`; that accuracy failure remains open. Physical smoothness, cold coordinator startup during speech, full installed Voice UI interaction, native Codex/Claude launches and local inference remain unproven. No Lenovo delivery follows from these local checks.

The [GPT-Live Mac experiment](handoffs/2026-09-13-gpt-live-mac-experiment.md), [OpenAI Realtime checkpoint](handoffs/2026-09-13-openai-realtime-mac-acceptance.md), and [LiveKit audio checkpoint](handoffs/2026-09-13-livekit-mac-acceptance.md) remain account/audio comparison baselines. Private testers on ports 57517, 57518, and 57519 retain separate RAM-only account values; do not restart them casually or ask for keys in chat. The user favors a small task dispatcher using supported agent interfaces, existing launch commands, and UI paste as a fallback; the current Linux candidate still uses Hermes as the task coordinator, so direct named-agent routing remains a separate unimplemented simplification.

The [earlier native verification](handoffs/2026-09-13-voice-native-verification.md) records the initial 175-test installed check and reproduced disabled-orb defect before native GPT-Live integration. Its statement that GPT-Live is absent from the production factory is historical. Published staging sequence 4 / Hub 0.3.0 and the frozen unsigned Hub 0.3.1 / Voice 0.1.3 candidate remain unchanged. Nothing was published or changed on Lenovo for these local tests. The [support-readiness closeout](handoffs/2026-09-12-support-readiness-closeout.md) remains the next infrastructure workstream.
