...
Delivery remains the earlier [Hub 0.3.0 / staging sequence 4 Voice update](handoffs/2026-09-13-maslow-voice-ota-delivery.md), with user-reported module arrival on Lenovo. The frozen unsigned Hub 0.3.1 / Voice 0.1.3 candidate is preserved. The new Hub 0.3.2 preparation carries the exact previously tested GPT-Live Voice 0.1.4-9, not the later LiveKit package. Signing/publication status belongs to the new test-release handoff; no new public delivery is established yet. The [initial implementation](handoffs/2026-09-13-maslow-voice-implementation.md) and [setup correction](handoffs/2026-09-13-livekit-setup-correction.md) remain historical provenance.

Resume Voice through [the Voice development/testing guide](../agents/skills/voice-development.md) and the checkpoint above. Runtime integration is `codex/livekit-setup`; package integration is `codex/linux-live-packages`. Rediscover their worktrees with Git. The private Mac testers on ports 57517/57518/57519 use independent RAM-only account values; do not restart them casually or request keys in chat. The prior credentialed guest cleanup removed guest credentials. The new OTA rehearsal uses a separate overlay with no new provider credentials; its final lifecycle and cleanup are recorded in the test-release handoff. Direct named-agent routing remains a proposed simplification, not the implemented Hermes coordinator path. The [support-readiness closeout](handoffs/2026-09-12-support-readiness-closeout.md) remains the separate infrastructure workstream.
...
Current delivery checkpoint: the new [GPT-Live Lenovo test release](handoffs/2026-09-14-gpt-live-lenovo-test-release.md) supersedes the frozen 0.3.1 candidate for the authorized next update. Public Hub 0.3.0 / staging sequence 4 remains signed and anonymously verified. The user now reports the update and Voice module are visible on Lenovo, but a real LiveKit conversation was not tested. The newer [LiveKit correction handoff](handoffs/2026-09-13-livekit-setup-correction.md) supersedes its setup guidance and records the unsigned Hub 0.3.1 / Voice 0.1.3 candidate. A3 physical Lenovo recovery and F1 Observability backend/trace acceptance remain open; Voice live-provider/audio and orb appearance are separate gates. R1/R2 remain recommendations, and security design S1 still blocks any promise of agent-hidden secrets.
...
Public baseline while the [GPT-Live test release](handoffs/2026-09-14-gpt-live-lenovo-test-release.md) awaits signing/publication: signed staging sequence 4 / catalog sequence 1, distribution `bfd869fbb4da6776abe24fff58de01fe3afe20b7`, anonymously verified September 13 at 19:16:59 UTC. Immutable versions 0.1.3, 0.2.0, 0.2.1 and 0.3.0 are available; metadata expires September 24, 2026 at 23:56:41 UTC. Mac signing and publication are complete. The later user report establishes arrival of the Voice update/module, while LiveKit setup review found defects now corrected in a new unsigned candidate. Follow the [LiveKit correction handoff](handoffs/2026-09-13-livekit-setup-correction.md) for current work and the [Voice OTA handoff](handoffs/2026-09-13-maslow-voice-ota-delivery.md) for the completed public release; existing trust enrollment is reused.
