...
Voice implementation: [Maslow Voice implementation and acceptance](handoffs/2026-09-13-maslow-voice-implementation.md) records the separate `codex/maslow-voice` runtime, Hub and package worktrees, source changes, internal package builds, real Quickshell and installed QEMU evidence, and remaining live-provider/hardware gates. The scoped [Voice Hub 0.3.0 OTA delivery handoff](handoffs/2026-09-13-maslow-voice-ota-delivery.md) records the final unsigned archive, verified test-channel update and rollback, exact release pins and completed corrected-candidate installed checks. Resume the release work there. It has not been merged, signed or published and does not replace the Hub release baseline below.

Current priority: use the pinned Mac signing command in the [Voice Hub 0.3.0 OTA delivery handoff](handoffs/2026-09-13-maslow-voice-ota-delivery.md), then continue its existing public staging, anonymous verification and Lenovo Hub flow. The native test-channel update, rollback, corrected reapply, final visuals and cleanup passed; public staging remains at sequence 3 and candidate sequence 4 is unsigned and unpublished. After that bounded delivery, resume the [support-readiness closeout, RCA and next steps](handoffs/2026-09-12-support-readiness-closeout.md). Remote support infrastructure remains proposed, not implemented or authorized by this documentation closeout.
...
Current checkpoint: A2 staging delivery and 0.2.1 publication are complete; Lenovo visibly reports 0.2.1. For the Voice OTA, the initial signed test-channel `0.1.3 -> 0.2.1 -> 0.3.0` update, normal Hub rollback to 0.2.1 and final corrected 0.3.0 reapply passed while preserving Voice, Hermes, the service, core versions, launcher state and Quickshell in the pre-crash session. QEMU later exited with SIGSEGV without a container OOM; the preserved overlay rebooted successfully, and the corrected settled Hub screen and final installed state passed inspection with one new Quickshell process. Earlier installed Settings evidence remains valid because the final candidate changes only What's New copy. Canonical guest shutdown and owned-container cleanup passed. The final archive remains unsigned and unpublished; the next action is the pinned interactive Mac signing command. A3 physical Lenovo recovery and F1 Observability backend/trace acceptance remain open; Voice live-provider/audio acceptance is a separate open gate. R1/R2 are recommended next, not authorization to deploy or collect device data. Security design S1 still blocks any promise of agent-hidden secrets.
...
Current publication record: signed staging sequence 3 / catalog sequence 1, distribution `8791c37972525af0edcbe6ee980d70b7f97d4577`, anonymously reverified September 13. Versions 0.1.3, 0.2.0 and 0.2.1 remain immutable; metadata expires September 24, 2026 at 23:56:41 UTC. Lenovo's supplied photo shows 0.2.1. Voice candidate manifest sequence 4 is prepared locally but remains unsigned and unpublished; catalog sequence 1 and the existing package signatures are retained. No further signing or enrollment is needed for the existing 0.2.1 update. Read the [Voice OTA handoff](handoffs/2026-09-13-maslow-voice-ota-delivery.md) before acting on the candidate, and read the latest support closeout for the public baseline.
