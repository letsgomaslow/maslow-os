...
Current priority: the [LiveKit installed handoff checkpoint](handoffs/2026-09-14-livekit-installed-handoff.md) records two fixed Linux defects and the new unpublished Voice 0.1.4-11 candidate. A real typed LiveKit request now reaches Hermes, creates the exact artifact, and receives an independent typed conversation reply while work is active, with one task and zero provider errors. This includes normal first-task coordinator startup without prewarming. Spoken acceptance remains blocked: the installed QEMU/TCG guest drops microphone frames, including with graphics paused; the unchanged quality checks failed. Do not equate typed handoff with spoken or Lenovo readiness. Next establish reliable native audio delivery and pass the original spoken request, concurrent reply and soak with the desktop active before delivery.
