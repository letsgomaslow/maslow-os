...
Current priority: follow the [LiveKit Mac acceptance checkpoint](handoffs/2026-09-13-livekit-mac-acceptance.md) in the new `codex/livekit-setup` runtime worktree. Three-field setup and microphone/error handling are corrected. A real LiveKit speech round trip passed; the user reported choppy audio, then improvement after the reproduced PCM playback gap was fixed. Independent LiveKit source review supports that diagnosis. Local commits `df153202` and `2b46a3d9` retain the fixes and private Mac voice chooser; next is account reentry into the restarted tester, auditioning voices, and selecting a preference. The [earlier LiveKit setup correction](handoffs/2026-09-13-livekit-setup-correction.md) records the frozen unsigned Hub 0.3.1 / Voice 0.1.3 candidate; it was never published, explaining why Lenovo cannot see it. Published staging sequence 4 / Hub 0.3.0 remains unchanged. Finish live testing before preparing another update or asking the user to sign. Broader physical audio quality, installed proof of the follow-up source, and the flat orb remain open. The separate [support-readiness closeout](handoffs/2026-09-12-support-readiness-closeout.md) remains the next infrastructure workstream.
