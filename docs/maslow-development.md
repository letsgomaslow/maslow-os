...
Current priority: follow the [GPT-Live Mac experiment](handoffs/2026-09-13-gpt-live-mac-experiment.md) in `codex/livekit-setup`. The user confirmed GPT-Live felt smooth and responsive on the native Mac; actual synthetic speech and Quartz preview checks also passed. Actual typed and spoken requests subsequently produced verified scratch HTML artifacts through the separate Hermes runner; the synthetic spoken scenario observed conversation output while its task ran, and a real running-task cancellation preserved the prior artifact. The user now favors a simpler first slice: a small service that sends a prepared task directly to the named agent using supported interfaces, with existing app launch commands/shortcuts and UI paste as a fallback. Hermes is not a mandatory intermediate planner for a named destination. The [OpenAI Realtime checkpoint](handoffs/2026-09-13-openai-realtime-mac-acceptance.md) and [LiveKit audio checkpoint](handoffs/2026-09-13-livekit-mac-acceptance.md) remain comparison baselines. Private testers are on ports 57517 (LiveKit), 57518 (Realtime), and 57519 (GPT-Live), with separate RAM-only account values; do not restart them casually or ask for keys in chat. Published staging sequence 4 / Hub 0.3.0 and the frozen unsigned Hub 0.3.1 / Voice 0.1.3 candidate remain unchanged. Finish the selected local handoff experiment and queued flat orb before preparing another update. Installed proof and Lenovo acceptance remain open. The separate [support-readiness closeout](handoffs/2026-09-12-support-readiness-closeout.md) remains the next infrastructure workstream.

Native verification checkpoint: the [current Voice native verification](handoffs/2026-09-13-voice-native-verification.md) records the exact Docker/Weston/Sway/Quickshell route and a fresh disposable installed x86_64 overlay. Current Voice source passed 175 tests with two Mac-only sandbox skips inside the installed guest; one portable test-fixture correction is committed as `34661948`. The disabled shader defect was reproduced. GPT-Live remains a private Mac experiment, absent from the production provider factory/settings, so the integrated Linux conversation-to-selected-agent flow is still an open gate. No package or channel was published.
