...
Current priority: follow the [OpenAI Realtime Mac acceptance checkpoint](handoffs/2026-09-13-openai-realtime-mac-acceptance.md) in `codex/livekit-setup`. The user confirmed LiveKit auditions worked after the [PCM playback fix](handoffs/2026-09-13-livekit-mac-acceptance.md), and requested OpenAI testing before update preparation. The real OpenAI synthetic speech check and Cedar/Marin speaker-only previews passed; live testing found and corrected a rejected preview message identifier. Both providers now have saved native voice choices. The private OpenAI page is at `http://127.0.0.1:57518/`, with the user's key already in RAM; LiveKit remains on port 57517 with its connection preserved. Next is the user's actual microphone/speaker and interruption feedback. Do not restart either tester casually or ask for keys in chat. Published staging sequence 4 / Hub 0.3.0 and the frozen unsigned Hub 0.3.1 / Voice 0.1.3 candidate remain unchanged. Finish live testing and the queued flat orb before preparing another update or asking the user to sign. Installed proof of this follow-up source and Lenovo acceptance remain open. The separate [support-readiness closeout](handoffs/2026-09-12-support-readiness-closeout.md) remains the next infrastructure workstream.
