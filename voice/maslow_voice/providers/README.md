...
Secrets remain daemon-owned: `openai`, or `livekit_key` and `livekit_secret`; `server_token` is sent only to the configured user server. Fully offline mode receives no account credentials and routes model requests through the isolated worker. Local speech uses the daemon's `speech_directory`, containing the explicit `whisper-base.en` directory and `en_US-ljspeech-medium.onnx` Piper model. This layer neither downloads models nor executes a submitted intent.
