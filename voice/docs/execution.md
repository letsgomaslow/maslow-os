...
Direct Codex and Claude jobs are in-process and are explicitly `resumable = False`. If Voice restarts after a direct task reaches submitting, accepted, running, awaiting approval, or stopping, recovery writes `DIRECT_RUN_LOST` and marks it interrupted. It does not reconstruct the client, resume the adapter, or resubmit the request. A still-queued task is safe to start because no adapter received it. For direct Codex, Continue creates a new explicit attempt and resumes the saved thread through `thread/resume`; a missing or rejected thread fails explicitly. It never silently creates a replacement thread. Other adapters retain their own continuation capabilities.
...
For a direct peer run, `DirectExecutionClient` creates exactly one child using the persisted original request and brief. Its synthetic run identity is local to the current daemon attempt. Status is derived from that exact child; a new client cannot claim or rediscover an earlier direct run. Direct cloud/server Codex steering uses `turn/steer` with the exact active thread and expected turn ID. Spoken and typed corrections use the same serialized command path. A correction racing with completion fails and offers explicit Continue. Claude and offline adapters do not advertise unsupported steering controls.
